
export const services = [
    {
        id: '1',
        icon: "fal fa-tv",
        title: "Lorem Ipsum",
        text: "Voluptatum deleniti atque corrupti quos dolores et quas molestias excepturi sint occaecati cupiditate non provident"
    }, {
        id: '2',
        icon: "fal fa-chart-bar",
        title: "Dolor Sitema",
        text: "Minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat tarad limino ata"
    }, {
        id: '3',
        icon: "far fa-globe-europe",
        title: "Sed ut perspiciatis",
        text: "Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur"
    }, {
        id: '4',
        icon: "fal fa-cog",
        title: "Magni Dolores",
        text: "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum"
    }, {
        id: '5',
        icon: "fal fa-calendar-alt",
        title: "Nemo Enim",
        text: "At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque"
    }, {
        id: '6',
        icon: "far fa-image",
        title: "Eiusmod Tempor",
        text: "Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi"
    },
]

// Port-Folio-Image - Data
export const portfolioList = [
    {
        id: '0',
        name: "Sarfraz Alam",
        post: "Electrical Engineer",
        img: require('../assets/img/portfolio-1.jpg')
    },
    {
        id: '1',
        name: "Md Usman",
        post: "Electrical Engineer",
        img: require('../assets/img/portfolio-2.jpg')
    },
    {
        id: '2',
        name: "Md Suleman",
        post: "Store Manager",
        img: require('../assets/img/portfolio-3.jpg')
    },
    {
        id: '3',
        name: "Md Rizwan",
        post: "Frontend Developer",
        img: require('../assets/img/portfolio-4.jpg')
    },
    {
        id: '4',
        name: "Md Irfan",
        post: "Machenical Engineer",
        img: require('../assets/img/portfolio-5.jpg')
    },
    {
        id: '5',
        name: "Ayaan Ahmad",
        post: "Full-Stak Developer",
        img: require('../assets/img/portfolio-6.jpg')
    },
    {
        id: '5',
        name: "Faizan Ahmad",
        post: "Frontend Developer",
        img: require('../assets/img/portfolio-7.jpg')
    },
    {
        id: '6',
        name: "Faiyaz Alam",
        post: "Frontend Developer",
        img: require('../assets/img/portfolio-8.jpg')
    },
    {
        id: '7',
        name: "Affan Ahmad",
        post: "Full-Stack Developer",
        img: require('../assets/img/portfolio-9.jpg')
    },
]


export const CarsouelData = [
    {
        id: "0",
        name: "Sarfraz Alam",
        post: "Designer",
        image: require("../assets/card-images/sarfraz.jpg"),
        text: "Export tempor illum tamen malis malis eram quae irure esse labore quem cillum quid cillum eram malis quorum velit fore eram velit sunt aliqua noster fugiat irure amet legam anim culpa.",
    }, {
        id: "1",
        name: "Md Usman",
        post: "Freelancer",
        image: require("../assets/card-images/usmancover.jpg"),
        text: "Enim nisi quem export duis labore cillum quae magna enim sint quorum nulla quem veniam duis minim tempor labore quem eram duis noster aute amet eram fore quis sint minim.",
    }, {
        id: "2",
        name: "Md Irfan",
        post: "Store Owner",
        image: require("../assets/card-images/irfan4.jpg"),
        text: "Fugiat enim eram quae cillum dolore dolor amet nulla culpa multos export minim fugiat minim velit minim dolor enim duis veniam ipsum anim magna sunt elit fore quem dolore labore illum veniam.",
    }, {
        id: "3",
        name: "John Larson",
        post: "Entrepreneur",
        image: require("../assets/carousel/testimonials-4.jpg"),
        text: "Quis quorum aliqua sint quem legam fore sunt eram irure aliqua veniam tempor noster veniam enim culpa labore duis sunt culpa nulla illum cillum fugiat legam esse veniam culpa fore nisi cillum quid.",
    }, {
        id: "4",
        name: "Irfan Babu",
        post: "Ceo & Founder",
        image: require("../assets/card-images/irfan3.jpg"),
        text: "Proin iaculis purus consequat sem cure digni ssim donec porttitora entum suscipit rhoncus. Accusantium quam, ultricies eget id, aliquam eget nibh et. Maecen aliquam, risus at semper.",
    }, {
        id: "5",
        name: "Jena Karlis",
        post: "Store Owner",
        image: require("../assets/carousel/testimonials-1.jpg"),
        text: "Enim nisi quem export duis labore cillum quae magna enim sint quorum nulla quem veniam duis minim tempor labore quem eram duis noster aute amet eram fore quis sint minim.",
    },
]

// Data For My Friends List 

export const FriendsCarsouel = [
    {
        id: '0',
        img: require('../assets/clgcover.jpg'),
    }, {
        id: '1',
        img: require('../assets/img/portfolio-3.jpg'),
    }, {
        id: '2',
        img: require('../assets/img/portfolio-3.jpg'),
    }, {
        id: '3',
        img: require('../assets/img/portfolio-4.jpg'),
    }, {
        id: '4',
        img: require('../assets/img/portfolio-5.jpg'),
    }, {
        id: '5',
        img: require('../assets/img/portfolio-6.jpg'),
    }, {
        id: '5',
        img: require('../assets/img/portfolio-7.jpg'),
    }, {
        id: '6',
        img: require('../assets/img/portfolio-8.jpg'),
    }, {
        id: '7',
        img: require('../assets/img/portfolio-9.jpg'),
    },
]


// Cart Image Data 


export const CardData = [
    {
        id: "0",
        name: "Md Usman",
        post: "Designer",
        image: require("../assets/card-images/usmancover.jpg"),
        text: "Export tempor illum tamen malis malis eram quae irure esse labore quem cillum quid cillum eram malis quorum velit fore eram velit sunt aliqua noster fugiat irure amet legam anim culpa.",
    },
    {
        id: "1",
        name: "Md Suleman",
        post: "Designer",
        image: require("../assets/card-images/sulemancover.jpg"),
        text: "Export tempor illum tamen malis malis eram quae irure esse labore quem cillum quid cillum eram malis quorum velit fore eram velit sunt aliqua noster fugiat irure amet legam anim culpa.",
    },
    {
        id: "2",
        name: "Farukh Team08",
        post: "Designer",
        image: require("../assets/card-images/farukh2.jpeg"),
        text: "Export tempor illum tamen malis malis eram quae irure esse labore quem cillum quid cillum eram malis quorum velit fore eram velit sunt aliqua noster fugiat irure amet legam anim culpa.",
    }, {
        id: "3",
        name: "Afroj Alam",
        post: "Store Owner",
        image: require("../assets/card-images/afrojcover.jpg"),
        text: "Enim nisi quem export duis labore cillum quae magna enim sint quorum nulla quem veniam duis minim tempor labore quem eram duis noster aute amet eram fore quis sint minim.",
    }, {
        id: "4",
        name: "Sharukh Team08",
        post: "Freelancer",
        image: require("../assets/card-images/sharukh2.jpeg"),
        text: "Fugiat enim eram quae cillum dolore dolor amet nulla culpa multos export minim fugiat minim velit minim dolor enim duis veniam ipsum anim magna sunt elit fore quem dolore labore illum veniam.",
    }, {
        id: "5",
        name: "Mukesh Verma",
        post: "Entrepreneur",
        image: require("../assets/card-images/mukesh.JPG"),
        text: "Quis quorum aliqua sint quem legam fore sunt eram irure aliqua veniam tempor noster veniam enim culpa labore duis sunt culpa nulla illum cillum fugiat legam esse veniam culpa fore nisi cillum quid.",
    }, {
        id: "6",
        name: "Aashish Badal",
        post: "Store Owner",
        image: require("../assets/card-images/aashish.jpeg"),
        text: "Enim nisi quem export duis labore cillum quae magna enim sint quorum nulla quem veniam duis minim tempor labore quem eram duis noster aute amet eram fore quis sint minim.",
    }, {
        id: "7",
        name: "Saurav Morya",
        post: "Store Owner",
        image: require("../assets/card-images/saurav.jpg"),
        text: "Enim nisi quem export duis labore cillum quae magna enim sint quorum nulla quem veniam duis minim tempor labore quem eram duis noster aute amet eram fore quis sint minim.",
    }, {
        id: "8",
        name: "Ravi Mandal",
        post: "Store Owner",
        image: require("../assets/card-images/ravi2.jpeg"),
        text: "Enim nisi quem export duis labore cillum quae magna enim sint quorum nulla quem veniam duis minim tempor labore quem eram duis noster aute amet eram fore quis sint minim.",
    }
]